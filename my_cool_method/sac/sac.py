...
from my_cool_method.sac import models
from my_cool_method.sac.exploration import ExplorationHelper
from my_cool_method.sac.replay_buffers import ReplayBuffer, ReservoirReplayBuffer, PrioritizedReplayBuffer, BufferType, \
    PrioritizedExperienceReplay
from my_cool_method.utils.logx import EpochLogger
from my_cool_method.utils.run_utils import reset_optimizer, reset_weights, set_seed, create_one_hot_vec
from my_cool_env.env.scenario.common import CommonEnv
    ...
    def run(self):
        ...
        for global_timestep in range(self.steps):
            ...
            if current_task_timestep >= self.update_after and current_task_timestep % self.update_every == 0:
                ...
                self.logger.log(f"Time elapsed for a policy update: {time.time() - time_update_start}", color='blue')
